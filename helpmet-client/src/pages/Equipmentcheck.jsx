...
const EquipmentCheck = () => {
  ...
  const fetchEquipments = async () => {
    try {
      const response = await axios.get(
        `http://52.53.246.102:5001/companies/${companyID}/equipments`
      );
  ...
  const handleUpdateEquipment = async (updatedEquipment) => {
    try {
      const response = await axios.put(
        `http://52.53.246.102:5001/equipments/${updatedEquipment.equipmentID}`,
        updatedEquipment
      );
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(
        `http://52.53.246.102:5001/companies/${companyID}/equipments/${equipmentToDelete}`
      );
      setEquipments((prevEquipments) =>
  ...
  const handleViewEquipment = async (equipmentID) => {
    try {
      const response = await axios.get(
        `http://52.53.246.102:5001/equipments/${equipmentID}`
      );
      if (response.status === 200) {
