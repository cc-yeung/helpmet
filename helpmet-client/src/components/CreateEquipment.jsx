...
const CreateEquipment = ({ isOpen, onSave, onCancel }) => {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await axios.get(`http://18.144.11.61:5001/companies/${companyID}/employees`);
  ...
  const fetchLocations = async () => {
    try {
      const response = await axios.get(`http://18.144.11.61:5001/companies/${companyID}/locations`);
  ...
  const handleCreateEquipment = async (equipment) => {
    try {
      await axios.post(`http://18.144.11.61:5001/companies/${companyID}/equipments`, equipment);
      onSave();
    } catch (error) {
